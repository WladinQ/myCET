jQuery(function(){
	...
	// doplnenie aula 5 koniec
	
	// js obraz okna na otvorenie
	function add_janela(id, nome, status){
		...
		janela += '<div class="header_window"><a href="#" class="close">X</a> <span class="name"><a href="'+nome+'">'+nome+'</a></span> <span id="'+id_user+'" class="'+status+'" title="'+status+'"></span></div>';
		janela += '<div class="body"><div class="mensagens"><ul></ul></div>';
		janela += '<div class="send_message" id="'+id+'"><input type="text" name="mensagem" class="msg" id="'+id+'" placeholder="Napsat zprávu..."></div></div></div>';
		
		jQuery('#chats').append(janela);
	}
	// koniec obrazu okna na otvorenie
	
	// js na historiu konverzacie
	...
	// koniec js na historiu konverzacie
	
	// js na otvorenie okna
	...
	// koniec js na otvorenie okna
	
	// js funkcia na min. a max. okna
	jQuery('body').on('click', '.header_window', function(){
	  var next = jQuery(this).next();
	  next.toggle(100);
	});
	// koniec js funkcie na min. a max. okna
	
	// js funkcia na zavretie okna pomocou X
	...
	// koniec js na zavretie okna pomocou X
	
	// js na odosielanie sprav do DB
	    ...
	    jQuery.ajax({
	      ...
	      success: function(retorno){
	        if(retorno == 'ok'){
				...
				msgEl.className = 'eu';
				msgEl.textContent = jQuery('.msg').val();
				jQuery(<chat-window>).append(msgEl);
				*/
				jQuery('.msg').val('');
	        }else{
	          alert("Došlo k chybe pri odosielaní správy!");
	        }
	      }
	    });
	  }
	});
	// koniec js na odosielanie sprav do DB

/* up data vporiadku */

	// aula 5 vecny problem zaciatok
			...
			success: function(retorno){
				...
				if(retorno.status == 'resultados' || retorno.status == 'vazio'){
					...
					*/
					...
					alert('Niečo sa pokazilo, aktualizujte stránku.');
				}
			},
			error: function(){
				clearInterval(t);
